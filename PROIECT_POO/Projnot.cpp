...
class Client {
  ...
  public:
    ...
    void scrie_in_fisier(string nume_fisier) {
      ...
      if (fout.is_open()) { // verificam daca fisierul s-a deschis cu succes
        fout << nume << "," << prenume << "," << CNP << endl; // scriem datele clientului pe prima linie
        for (int i = 0; i < tranzactii.size(); i++) { // scriem tranzactiile clientului pe liniile urmatoare
    ...
    void citeste_din_fisier(string nume_fisier) {
      ...
      if (fin.is_open()) { // verificam daca fisierul s-a deschis cu succes
        ...
        while (fin >> t) { // citim tranzactiile clientului din fisier
...
void scrie_nume_fisiere(string nume_fisier, vector<string> nume_fisiere) {
  ofstream fout(nume_fisier); // deschidem fisierul pentru scriere
  if (fout.is_open()) { // verificam daca fisierul s-a deschis cu succes
    for (int i = 0; i < nume_fisiere.size(); i++) { // scriem numele fisierelor pe cate o linie
...
vector<string> citeste_nume_fisiere(string nume_fisier) {
  ...
  if (fin.is_open()) { // verificam daca fisierul s-a deschis cu succes
    ...
    while (getline(fin, nume)) { // citim numele fisierelor din fisier
